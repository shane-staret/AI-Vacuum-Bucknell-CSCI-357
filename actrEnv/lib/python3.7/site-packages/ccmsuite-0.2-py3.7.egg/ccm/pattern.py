...
def get(obj,name,key):
  ...
  try:
      x=a[key]
  #Added TypeError to exception caught to fix issue that seemed to come w/ using Python 3
  except (AttributeError, TypeError):
      x=getattr(a,key)
...
class Pattern:
    ...
    def match(self,obj):
        ...
        try:
            for f in self.funcs:
                # This line was cause of original issue w/ children of the Model class not working
                #  Fixed the issue by changing the exception in the get function (see above)
                #  This must have been tied to a different exception being thrown
                #   in Python 3 than Python 2
