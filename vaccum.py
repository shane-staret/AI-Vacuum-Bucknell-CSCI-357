...
class VacuumAgent(ACTR):
	...
	'''def startSwirl(goal = "swirl ?radius"):
		for i in radius:
			motorInst.go_forward()
			motorInst.clean_if_dirty()
			if(self.body.ahead_cell.wall:True)
				goal.set("wall")
			else if(self.body.cell.dirty:True)
				clean_cell()
				goal.set("swirl 0")
			else
				radius= str(int(radius)+i)
				goal.set("swirl radius")'''


	def WallFollow(goal="wall"):
		#after roomba encounters a wall
		motorInst.turn_left(2)
		for i in range(6):
			motorInst.go_forward()
			motorInst.clean_if_dirty()
			if(body.ahead_cell.wall):
				motorInst.turn_left()
		goal.set("roomba")

	def roombaPattern(goal= "roomba"):
			motorInst.clean_if_dirty()
			motorInst.go_forward()
			if(body.ahead_cell.wall):
				#motorInst.turn_left(2)
				...
				if direction == 'right':
					print("here")
					motorInst.turn_right(random.randint(1,7))
				else:
					print('there')
